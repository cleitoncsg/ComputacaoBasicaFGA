/**** Exercicio 07
O código está mais completo em relação a resolução Z e X.
Veja a brincadeira com os índices  de algumas funções****/
...
void numero_salario_operario(int codigo_operario[],int numero_pecas_produzidas[]){
	//Variáveis
	float salario[LOOPS]; int c;
	
	//Laço para imprimir os dados desejados
	for(c = 0; c < LOOPS; c++){
		//Condição para cálculo do salário da classe 1
		if(numero_pecas_produzidas[c] > 0 && numero_pecas_produzidas[c] <= 30) 
			salario[c] =  SALARIO_MINIMO; 
		
		//Condição para cálculo do salário da classe 2
		if(numero_pecas_produzidas[c] > 30 && numero_pecas_produzidas[c] <= 50)
			salario[c] = SALARIO_MINIMO + (numero_pecas_produzidas[c] - 30)*SALARIO_MINIMO*0.01; 
		
		//Condição para cálculo do salário da classe 3
		if(numero_pecas_produzidas[c] > 0 && numero_pecas_produzidas[c] > 50)
			salario[c] = ( SALARIO_MINIMO + (numero_pecas_produzidas[c] - 30)*SALARIO_MINIMO*0.02 ); 
	
		//Impressões de resultados de acordo com quantida de LOOPS ou operários
		printf("\nO operario com numero ou codigo %d tem o salario de R$ %.2f\n",codigo_operario[c],salario[c]);
	}//fim do laço
}//fim do procedimento
...
float total_folha_pagamento(int numero_pecas_produzidas[]){
	//Variáveis
	float salario[LOOPS],salario_total = 0; int percorre;
	
	//Laço para imprimir os dados desejados
	for(percorre = 0; percorre < LOOPS; percorre++){
		//Condição para cálculo do salário da classe 1
		if(numero_pecas_produzidas[percorre] > 0 && numero_pecas_produzidas[percorre] <= 30)
			salario[percorre] =  SALARIO_MINIMO;
		//Condição para cálculo do salário da classe 2
		if(numero_pecas_produzidas[percorre] > 30 && numero_pecas_produzidas[percorre] <= 50)
			salario[percorre] = SALARIO_MINIMO + (numero_pecas_produzidas[percorre] - 30)*SALARIO_MINIMO*0.01;
		//Condição para cálculo do salário da classe 3
		if(numero_pecas_produzidas[percorre] > 0 && numero_pecas_produzidas[percorre] > 50){
			salario[percorre] = SALARIO_MINIMO + (numero_pecas_produzidas[percorre] - 30)*SALARIO_MINIMO*0.02;
		}
		salario_total = salario_total + salario[percorre];
	}
...
int numero_pecas(int numero_pecas_produzidas[]){
	//Variáveis 
	int pecas_produzidas = 0,correndo_no_laco;
	//Loop para ajudar a percorrer todos os números de peças produzidas
	for(correndo_no_laco = 0; correndo_no_laco < LOOPS; correndo_no_laco++){
		//Guardando o numero de peças produzidas
		pecas_produzidas = pecas_produzidas + numero_pecas_produzidas[correndo_no_laco];
	}
	
	return pecas_produzidas; //retorna psara o main o valor de pecas_produzidas
}

//E se na fábrica tiver mulher igual no Campus do Gama?
//Haverá uma divisão por zero. Vamos arrumar isso, ne. hehe
...
//E se na fábrica tiver mulher igual no Campus do Ceilâncdia?
//Haverá uma divisão por zero. Vamos arrumar isso, ne. hehe² (Nem eu ri)
...
//Na resolução Z, vamos imprimir se é operário ou operária, ou seja, vamos considerar o sexo
//Procedimento para mostarr o código do operário ou operária de maior salário > letra F
void codigo_maior_salario(int numero_pecas_produzidas[],int codigo_operario[],char sexo[]){
	//Variáveis
	float salario[LOOPS]; float maior_salario = 0; int guarda_codigo = 0; 
	int i; 
	char guarda_sexo = 'C'; // A variável pode ser igualada a qualquer caracter de dimensão 1 no ínicio
	...
	for(i = 0; i < LOOPS; i++){
		...
		if(salario[i] > maior_salario){
			...
			guarda_sexo = sexo[i]; //guardando o sexo do operário
		}

	}
	//Impressão do código do maior salário do operário ou operária de acordo com as condições
	if(guarda_sexo == 'm' || guarda_sexo == 'M')
		printf("\nO codigo do operario de maior salario = %d\n",guarda_codigo);
	if(guarda_sexo == 'f' || guarda_sexo == 'F') {
		printf("\nO codigo da operaria de maior salario = %d\n",guarda_codigo); }
}

//Procedimento ate
void ate_a_proxima(){
	system("cls"); //Apagar a tela do executável
	system("color 1A"); 
...
int main(){
	...
	codigo_maior_salario(numero_pecas_produzidas,codigo_operario,sexo);
	printf("-_--_--_-_--_--_-_--_--_-_--_--_-_--_--_-_--_--_-_--_--_-_--_--_-_--_--_-_--_--_\n");
	
	printf("\nPressione qualquer tecla para encerrar o software:\n"); getch();
	ate_a_proxima(); //Chamada do procedimento tchau()
}
